...
barchart_qps <- function(dataset) {
  ...
  p <- ggplot(plotdata, aes(
    x = reorder_within(algorithm, qps, workload),
    y = qps,
    fill = algorithm
  )) +
    geom_col() +
    geom_hline(
      mapping = aes(yintercept = breaks),
      data = breaks,
      color = "white",
      lwd = .5
    ) +
...
distribution_normalized_latency <- function(dataset) {
  ...
  max_val <- non_outliers %>%
    pull(normalized_query_time) %>%
    max()
...
distribution_latency <- function(dataset) {
  ...
  plotdata <- dataset %>%
    mutate(
      algo_wpar = str_c(algorithm, algorithm_params, sep = "."),
  ...
  non_outliers %>%
    ...
    geom_text(aes(
      label = scales::number(time_90, prefix = "→ "),
      y = algorithm
    ),
    x = max_val,
    hjust = 1,
...
plot_overview2 <- function(best, metric, xlab, n_bins = 60, annotations_selector = which.max) {
  ...
  p <- plotdata %>%
    ...
    geom_text_interactive(aes(
      x = algo_id - 0.1, y = bin, data_id = data_id,
    ...
    scale_y_continuous(
      trans = "log", breaks = c(1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000),
      labels = scales::number_format(),
      expand = expansion(add = .5)
    ) +
    labs(
      y = xlab, # The plot is flipped, hence y -> x
      x = "algorithm",
      color = "workload"
    ) +
    coord_flip() +
...
save_png <- function(ggobj, filename, width = 10, height = 6) {
  ...
  knitr::include_graphics(here::here(filename))
}
...
plot_top_query_times <- function(querystats) {
  querystats %>%
    ...
    geom_text(aes(
      label = scales::percent(fraction_total_query_time),
