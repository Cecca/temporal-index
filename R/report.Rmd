...
2. How robust are the algorithms to changes in the query workload? I.e., is the query time stable?
3. How are query times distributed? 
...
^[First entry on 2020-07-31, updated after changing the generation of datasets to remove deduplication, so that we actually get the distributions we asked for and the number of intervals we asked for.Furthermore, the change introduced the a separate parameterization of the duration of the query.]]
...
The three dimensional grid is by far the best performing algorithm.
Now the `PeriodIndex` performs better, but still not as well as the three dimensional grid or the `BTree`
...
The plot above meets the expectations, but the throughput looks a little bit too low.
...
# Duration only queries [2020-08-29]

My expectations here are for `BTree` to perform the best.
